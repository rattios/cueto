...
export class AprobarPagosComponent implements OnInit {
	...
	ngOnInit() {
		this.loading=true;
		this.http.get(this.ruta.get_ruta()+'public/recibos?estado=R')
         .toPromise()
         .then(
           data => { // Success
             console.log(data);
	...
	mesesReverso(mes){
	  ...
	  aprobar(){
        ...
        var send2={
        	anio:this.item.anio,
        	mes:this.mesesReverso(this.item.mes),
        	cliente_id:this.item.cliente.id,
        	sucursal_id:this.item.sucursal_id,
	  		estado:'A',
	  		monto: this.importe,
	  		recibo_id: this.item.id
	  	};
	  	this.http.post(this.ruta.get_ruta()+'public/pago',send2)
         .toPromise()
         .then(
           data => { // Success
             console.log(data);
             this.loading=false;
             this.showNotification('top','center','Registrado el pago con éxito.',1);
           },
           msg => { // Error
             console.log(msg);
             this.loading=false;
